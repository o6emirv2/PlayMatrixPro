...
router.post('/wheel/spin', requireAuth, async (req, res) => {
  ...
  if (db) {
    ...
    const index = REWARDS.findIndex((item) => item.amount === reward.amount);
    ...
    await db.collection('users').doc(uid).set({ lastSpin: Date.now(), lastSpinAt: Date.now(), updatedAt: Date.now() }, { merge: true }).catch(() => null);
    return res.json({ ok: true, day, reward: reward.amount, prize: reward.amount, amount: reward.amount, index, lastSpin: Date.now(), lastSpinAt: Date.now(), balance: economy.balance });
  }
  ...
  res.json({ ok: true, day, reward: reward.amount, prize: reward.amount, amount: reward.amount, index, lastSpin: Date.now(), lastSpinAt: Date.now(), balance: economy.balance });
});
