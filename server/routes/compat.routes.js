...
function defaultProfile(req, uid, seed = {}) {
  ...
  return addProgression({ uid, email, username, fullName: seed.fullName || username, displayName: username, avatar: seed.avatar || DEFAULT_AVATAR, selectedFrame: Number(seed.selectedFrame || 0) || 0, balance: Number(seed.balance ?? 50000) || 0, signupBonusClaimed: true, xp: 0, accountXp: 0, monthlyActiveScore: 0, totalRounds: 0, createdAt: now(), lastLogin: now(), lastSeen: now(), gameStats: { total: { rounds: 0, wins: 0, losses: 0, winRatePct: 0 }, snake: {}, space: {}, patternMaster: {}, classic: {} } });
}
...
async function addBalance(uid, amount, reason, key) {
  return adjustBalance({ uid, amount, reason, idempotencyKey: key, runtimeStore });
}
