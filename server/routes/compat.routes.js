...
function addProgression(profile) {
  ...
  return { ...profile, xp: progression.currentXp, accountXp: progression.currentXp, accountXpExact: progression.currentXpExact, accountLevel: progression.accountLevel, level: progression.accountLevel, accountLevelProgressPct: progression.accountLevelProgressPct, progressPercent: progression.progressPercent, currentLevelStartXp: progression.currentLevelStartXp, nextLevelXp: progression.nextLevelXp, xpIntoLevel: progression.xpIntoLevel, xpToNextLevel: progression.xpToNextLevel, isMaxLevel: progression.isMaxLevel, accountProgressionVersion: progression.accountProgressionVersion, accountLevelCurveMode: progression.accountLevelCurveMode, progression };
}
...
function sessionFromReq(req){ const cookieToken = String(req.headers.cookie || '').split(';').map(x=>x.trim()).find(x=>x.startsWith('pm_session=')); const token=String(req.headers['x-session-token'] || (cookieToken ? decodeURIComponent(cookieToken.slice('pm_session='.length)) : '') || '').trim(); if(!token)return null; const session=runtimeStore.temporary.get(`session:${token}`); return session?.uid?{token,...session}:null; }
...
router.get('/platform/control', requireAuth, (_req, res) => res.json({ ok: true, maintenance: false, gamesEnabled: true, removedOnlineGames: ['crash','chess','pisti'] }));
