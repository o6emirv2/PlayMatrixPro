...
function defaultProfile(req, uid, seed = {}) {
  ...
  return addProgression({ uid, email, username, fullName: seed.fullName || username, displayName: username, avatar: seed.avatar || DEFAULT_AVATAR, selectedFrame: Number(seed.selectedFrame || 0) || 0, balance: Number(seed.balance ?? 50000) || 0, signupBonusClaimed: true, xp: 0, accountXp: 0, monthlyActiveScore: 0, totalRounds: 0, createdAt: now(), lastLogin: now(), lastSeen: now(), gameStats: { total: { rounds: 0, wins: 0, losses: 0, winRatePct: 0 }, classic: {} } });
}
...
function sessionTokenFromReq(req){
  ...
  const part = cookie.split(';').map(x => x.trim()).find(x => x.startsWith('pm_session='));
  return part ? decodeURIComponent(part.slice('pm_session='.length)) : '';
}
function sessionFromReq(req){ const token=sessionTokenFromReq(req); if(!token)return null; const session=runtimeStore.temporary.get(`session:${token}`); return session?.uid?{token,...session}:null; }
function createAdminRuntimeSession({ uid='', email='', source='admin_matrix' } = {}){
  const safeUid = String(uid || '').trim();
  const safeEmail = String(email || '').trim().toLowerCase();
  if (!isConfiguredAdmin(safeEmail, safeUid)) return null;
  const sessionToken = crypto.randomBytes(32).toString('hex');
  const sessionId = crypto.randomBytes(12).toString('hex');
  runtimeStore.temporary.set(`session:${sessionToken}`, { uid: safeUid, email: safeEmail, sessionSource: source, sessionId, at: now() }, 12 * 3600000);
  const clientKey = issueClientKey({ uid: safeUid, email: safeEmail, sessionId });
  return { sessionToken, sessionId, clientKey, admin: adminContext(safeUid, safeEmail), user: { uid: safeUid, email: safeEmail } };
}
function setAdminSessionCookie(res, sessionToken=''){
  res.setHeader('Set-Cookie', `pm_session=${encodeURIComponent(sessionToken)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=43200; Secure`);
}
...
router.post('/auth/admin/matrix/step-name',strictLimiter,(req,res)=>{
  const payload=verifyMatrixToken(req.body?.ticket||'');
  if(!payload||payload.typ!=='pm_admin_step'||Number(payload.stage)!==3||Number(payload.expiresAt||0)<now())return res.status(401).json({ok:false,error:'Güvenlik oturumu geçersiz.'});
  if(!verifyThirdFactor(req.body?.adminName||req.body?.name||''))return res.status(403).json({ok:false,error:'Son güvenlik doğrulaması başarısız oldu.'});
  const session = createAdminRuntimeSession({ uid: payload.uid, email: payload.email, source: 'admin_matrix' });
  if(!session)return res.status(403).json({ok:false,error:'Yönetici yetkisi doğrulanamadı.'});
  setAdminSessionCookie(res, session.sessionToken);
  res.json({ok:true,redirectTo:'/admin/admin.html',sessionToken:session.sessionToken,session:{token:session.sessionToken,expiresAt:now()+12*3600000},clientKey:session.clientKey,admin:session.admin,user:session.user});
});
router.post('/auth/admin/bootstrap', strictLimiter, requireAuth, (req,res)=>{
  const uid = String(req.user?.uid || '').trim();
  const email = String(req.user?.email || '').trim().toLowerCase();
  const session = createAdminRuntimeSession({ uid, email, source: 'firebase_admin_bootstrap' });
  if(!session)return res.status(403).json({ok:false,authenticated:true,error:'ADMIN_REQUIRED'});
  setAdminSessionCookie(res, session.sessionToken);
  res.json({ok:true,authenticated:true,sessionToken:session.sessionToken,session:{token:session.sessionToken,expiresAt:now()+12*3600000},clientKey:session.clientKey,admin:session.admin,user:session.user});
});
router.get('/auth/admin/matrix/status',(req,res)=>{
  const session=sessionFromReq(req);
  if(!session||!isConfiguredAdmin(session.email,session.uid))return res.status(401).json({ok:false,authenticated:false,redirectTo:'/admin/index.html',error:'Yönetici oturumu bulunamadı.'});
  const rawKey = String(req.headers['x-admin-client-key']||'');
  const keyState=verifyClientKey(rawKey);
  const clientKey = keyState.ok ? rawKey : issueClientKey({uid:session.uid,email:session.email,sessionId:session.sessionId||''});
  res.json({ok:true,authenticated:true,user:{uid:session.uid,email:session.email},admin:adminContext(session.uid,session.email),clientKey});
});
router.post('/auth/admin/matrix/logout',(req,res)=>{ const token=sessionTokenFromReq(req); if(token)runtimeStore.temporary.delete(`session:${token}`); res.setHeader('Set-Cookie','pm_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0; Secure'); res.json({ok:true}); });
...
router.post('/auth/session/create', requireAuth, async (req, res) => { const token = crypto.randomBytes(32).toString('hex'); const uid = uidOf(req); runtimeStore.temporary.set(`session:${token}`, { uid, email: req.user?.email || '', sessionSource:'firebase_user', at: now() }, 30 * 86400000); res.setHeader('Set-Cookie', `pm_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000; Secure`); const user = await readProfile(req, uid, { email: req.user?.email || '' }); res.json({ ok: true, sessionToken: token, session: { token, uid, email: req.user?.email || '', expiresAt: now() + 30 * 86400000 }, user }); });
router.post('/auth/session/logout', (req, res) => { const token = s(req.headers['x-session-token'] || req.body?.sessionToken || sessionTokenFromReq(req), 100); if (token) runtimeStore.temporary.delete(`session:${token}`); res.setHeader('Set-Cookie','pm_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0; Secure'); res.json({ ok: true }); });
router.get('/auth/session/status', (req, res) => { const token = s(req.headers['x-session-token'] || req.query?.sessionToken || sessionTokenFromReq(req), 100); const session = token ? runtimeStore.temporary.get(`session:${token}`) : null; res.json({ ok: true, active: !!session, session: session ? { uid: session.uid || '', email: session.email || '', source: session.sessionSource || '', token } : null }); });
...
router.post('/profile/update', requireAuth, async (req, res) => { const uid = uidOf(req); const patch = { fullName: s(req.body?.fullName, 80), username: s(req.body?.username, 32), avatar: s(req.body?.avatar, 1000), selectedFrame: Math.max(0, Math.min(100, Math.floor(Number(req.body?.selectedFrame) || 0))) }; if (patch.username) patch.usernameLower = patch.username.toLowerCase(); Object.keys(patch).forEach(k => { if (patch[k] === '' && k !== 'selectedFrame') delete patch[k]; }); await writeProfile(uid, patch); res.json({ ok: true, user: await readProfile(req, uid) }); });
...
router.post('/update', requireAuth, async (req, res) => { const uid = uidOf(req); const patch = { fullName: s(req.body?.fullName, 80), username: s(req.body?.username, 32), avatar: s(req.body?.avatar, 1000), selectedFrame: Math.max(0, Math.min(100, Math.floor(Number(req.body?.selectedFrame) || 0))) }; if (patch.username) patch.usernameLower = patch.username.toLowerCase(); Object.keys(patch).forEach(k => { if (patch[k] === '' && k !== 'selectedFrame') delete patch[k]; }); await writeProfile(uid, patch); res.json({ ok: true, user: await readProfile(req, uid) }); });
