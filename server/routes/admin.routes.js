...
async function incrementBalance(uid, amount, reason, req) {
  ...
  const result = await adjustBalance({ uid, amount, reason, idempotencyKey: key, runtimeStore, actor: adminActor(req) });
  logAdmin(req, 'admin.balance.update', { uid, amount, reason, key, balance: result.balance, firestore: !!result.firestore });
  return result;
}
...
router.get('/admin/games', (_req, res) => {
  res.json({ ok: true, games: [
    { slug:'snake', title:'Snake Pro', status:'active', backend:'/server/games/snake/index.js', data:'score-validation' },
    { slug:'space', title:'Space Pro', status:'active', backend:'/server/games/space/index.js', data:'score-validation' },
    { slug:'pattern-master', title:'Pattern Master', status:'active', backend:'/server/games/pattern-master/index.js', data:'score-validation' }
  ], removedLiveGames: [
    { slug:'crash', title:'Crash', status:'static-card-only', frontend:'/static-games/crash/index.html' },
    { slug:'chess', title:'Satranç', status:'static-card-only', frontend:'/static-games/chess/index.html' },
    { slug:'pisti', title:'Pişti', status:'static-card-only', frontend:'/static-games/pisti/index.html' }
  ] });
});
