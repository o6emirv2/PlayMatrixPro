...
const ACTIVE_GAME_PAGES = Object.freeze({
  snake: 'snake',
  snakepro: 'snake',
  space: 'space',
  spacepro: 'space',
...
const STATIC_LEGACY_GAME_PAGES = Object.freeze({
  crash: 'crash',
  chess: 'chess',
  satranc: 'chess',
  'satranç': 'chess',
...
const REMOVED_LIVE_GAME_SLUGS = new Set(['crash', 'chess', 'satranc', 'satranç', 'pisti', 'pişti']);
...
app.get('/healthz', (_req, res) => res.json({ ok: true, service: 'playmatrix', env: env.nodeEnv, time: Date.now() }));
...
function sendActiveGamePage(slug, res, next) {
  const safeSlug = ACTIVE_GAME_PAGES[String(slug || '').toLowerCase()] || '';
...
function sendStaticGamePage(slug, res, next) {
  const safeSlug = STATIC_LEGACY_GAME_PAGES[String(slug || '').toLowerCase()] || '';
  if (!safeSlug) return next();
  res.setHeader('Cache-Control', 'no-store, max-age=0');
...
app.get('/games/:slug', (req, res, next) => {
  ...
  if (STATIC_LEGACY_GAME_PAGES[slug]) return res.redirect(302, `/static-games/${STATIC_LEGACY_GAME_PAGES[slug]}`);
...
io.on('connection', (socket) => {
  ...
  socket.on('client:error', (data) => {
    console.error('[socket:client:error]', JSON.stringify({ socketId: socket.id, data }));
    runtimeStore.errors.set(`socket_${Date.now()}_${socket.id}`, data || {}, 24 * 3600000);
  });
  socket.on('matchmaking:join', (data = {}) => {
    const game = String(data.game || '').toLowerCase();
    if (REMOVED_LIVE_GAME_SLUGS.has(game)) return socket.emit('matchmaking:error', { ok: false, error: 'LIVE_GAME_REMOVED', game });
    socket.emit('matchmaking:status', { ok: true, queued: true, game: game || 'unknown' });
  });
...
app.use((err, req, res, next) => {
  console.error('[server:error]', JSON.stringify({ message: err?.message || String(err), stack: err?.stack || '', path: req.originalUrl || req.url, method: req.method }));
